...
function configure (gyp, argv, callback) {
  ...
  var python = gyp.opts.python || process.env.PYTHON || 'python2'
    , buildDir = path.relative(process.cwd(), path.resolve('build'))
  ...
  function guessPython () {
    ...
    var pythonPath = path.relative(process.cwd(), path.resolve(rootDir, 'Python27', 'python.exe'))
  ...
  function getNodeDir () {
    ...
    if (gyp.opts.nodedir) {
      ...
      gyp.commands.install([ release.version ], function (err, version) {
        ...
        nodeDir = path.relative(process.cwd(), path.resolve(gyp.devDir, version))
        createBuildDir()
      })
    }
  }
  ...
  function createConfigFile (err) {
    ...
    var configPath = path.relative(process.cwd(), path.resolve(buildDir, configFilename))
  ...
  function findConfigs (err) {
    ...
    var fullPath = path.relative(process.cwd(),path.resolve(name))
  ...
  function runGyp (err) {
    ...
    var gyp_script = path.relative(process.cwd(),path.resolve(__dirname, '..', 'gyp', 'gyp_main.py'))
    var addon_gypi = path.relative(process.cwd(),path.resolve(__dirname, '..', 'addon.gypi'))
    var common_gypi = path.relative(process.cwd(),path.resolve(nodeDir, 'include/node/common.gypi'))

	
    fs.stat(common_gypi, function (err, stat) {
      if (err)
          common_gypi = path.relative(process.cwd(),path.resolve(nodeDir, 'common.gypi'))
      ...
      var nodeGypDir = path.relative(process.cwd(),path.resolve(__dirname, '..'))
      ...
      process.env.PYTHONPATH = path.relative(process.cwd(), path.resolve(__dirname, '..', 'gyp', 'pylib'))
