...
function build (gyp, argv, callback) {
  ...
  var makeCommand = gyp.opts.make || process.env.MAKE
    ...
    , command = (win && (gyp.opts.make == null)) ? 'msbuild' : makeCommand
  ...
  function doWhich () {
    ...
    which(command, function (err, execPath) {
      if (err) {
        if ((command == "msbuild") && /not found/.test(err.message)) {
  ...
  function copyNodeLib () {
    if (!(command == "msbuild") || !copyDevLib) return doBuild()
